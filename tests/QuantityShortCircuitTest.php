...
class QuantityShortCircuitTest extends TestCase
{
    ...
    public function testCustomPackedBoxSorterDisablesReplication(): void
    {
        ...
        $this->assertEquivalent($boxes, [[$item, 13]], false, $noPreferenceSorter);
        ...
        $packer = new Packer();
        $packer->setPackedBoxSorter($noPreferenceSorter);
        $packer->setLogger($packingIterations);
    ...
    public function testZeroVolumeTemplateHasNoArtificialVolumeLimit(): void
    {
        ...
        self::assertSame(5, $packingIterations->count, 'Zero-volume templates must not receive an artificial volume-derived cap');
    }
     ...
     */
    private function assertEquivalent(array $boxes, array $itemsWithQty, bool $beStrictAboutItemOrdering = false, ?PackedBoxSorter $sorter = null): void
    {
        $off = $this->pack($boxes, $itemsWithQty, false, $beStrictAboutItemOrdering, $sorter);
        $on = $this->pack($boxes, $itemsWithQty, true, $beStrictAboutItemOrdering, $sorter);
     ...
     */
    private function pack(array $boxes, array $itemsWithQty, bool $shortCircuit, bool $beStrictAboutItemOrdering, ?PackedBoxSorter $sorter = null): array
    {
        $packer = new Packer();
        if ($sorter !== null) {
