...
/**
 * Randomised differential harness for the quantity short-circuit optimisation, backported from the 4.x branch.
 *
 ...
 *
 * HISTORY: on the 4.x branch this harness established two things, both since fixed there and backported here.
 ...
 *
 * Adapted for 3.x: items here have only two rotation states (`keepFlat` true/false via {@see Rotation::KeepFlat} /
 * {@see Rotation::BestFit}) rather than 4.x's three-way `Rotation` enum (there is no `Rotation::Never` equivalent),
 * and leftover-tolerant packing is done via {@see InfalliblePacker} rather than `Packer::throwOnUnpackableItem()`
 ...
 * @phpstan-type ItemSpec array{description: string, w: int, l: int, d: int, weight: int, keepFlat: bool, qty: int}
...
class QuantityShortCircuitFuzzTest extends TestCase
{
     ...
     *
     * Uses {@see InfalliblePacker} (rather than throwing) since limited-supply boxes can legitimately run out,
     * leaving leftovers - comparing leftovers is part of the point.
     *
    ...
    private function buildItem(array $spec): TestItem
    {
        return new TestItem(
            ...
            $spec['d'],
            $spec['weight'],
            $spec['keepFlat'] ? Rotation::KeepFlat : Rotation::BestFit
        );
    }
     ...
     *
     * Every generated box can hold at least one copy of every generated item type (by both volume and weight). That
     * is a deliberate invariant: it keeps every generated box a genuine candidate throughout, so a capped item list
     * handed to a box evaluation is never empty and every scenario stays meaningful.
     *
    ...
    /**
     * 3.x items have only two rotation states (unlike 4.x's three-way Rotation enum, there is no equivalent of
     * Rotation::Never here), so the choice collapses to a single biased coin flip: ~40% keepFlat, ~60% not.
     */
    private function randomKeepFlat(): bool
    {
        return $this->chance(0.4);
    }
